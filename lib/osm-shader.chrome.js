...
class O3 {
	...
	constructor(canvas, options) {
		...
		var camera = new THREE.PerspectiveCamera( this.opts.camera.fov,
							this.s3.container.clientWidth / this.s3.container.clientHeight,
							this.opts.camera.near, this.opts.camera.far );
		camera.position.set( 0, 0, 1200 );
		camera.lookAt( scene.position );

		var container = this.s3.container;
	 ...
	 * @return {THREE.Mesh} segament mash or wireframe mesh
	 */
	...
	getRayTargets (matWorld, sects) {
		var castsGeom = new THREE.PlaneBufferGeometry (sects.x, sects.y, sects.w, sects.h);
