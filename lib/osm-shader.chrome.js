...
class O3 {
	...
	constructor(options) {
		...
		this.opts = {
			...
			position: {long: 104.09856, lat: 30.6765},	// center
		...
		// this.opts.lookAt = new THREE.Vector3();
		...
		// var campos = Vec3.minus(this.opts.position,
		// 				osmUtils.rad2cart(p.lon, p.lat, R));

		// var campos = [-options.h, 0, options.h];
		this.s3 = {campos};
	...
	getRayDirs (sects) {
		...
		for ( var ix = 0; ix < poses.count; ix++ ) {
			...
			v.applyMatrix4(this.s3.camera.matrixWorld);

			v = Vec3.minus(v, this.s3.camera.position);
	...
	onResize (s3) {
		...
		s3.renderer.setSize( window.innerWidth, window.innerHeight );
	}
...
class OsmUtils {
	...
	rad2cart(long, lat, a) {
		...
		var z = a * Math.sin(lat);
		var y = r * Math.sin(long);
		return {x : x, y: y, z: z};
	}
	...
	collectOsmTiles (tkeeper, dir, c0, a) {
		...
		var p = this.castPosition (c0, dir, a);
		if (p) {
