...
var GPUComputationRenderer = function ( renderer ) {
	...
	this.init = function (casto, cam) {
		...
		for ( var i = 0; i < this.variables.length; i ++ ) {
			...
			// var points = osmGlobeTiles(4, 500); // a = 500
			// var geometry = new THREE.BufferGeometry();
			// geometry.addAttribute( 'position', new THREE.BufferAttribute( points, 3 ) );
			// arg.mesh = new THREE.Mesh( geometry, arg.materail );
	...
	this.castingPointsBuff = function ( to, from, up, materail ) {
		...
		/**
		var grids = new Float32Array( (size.x + 1) * (size.y + 1) * 3 );
		var sx = w / (size.x + 1);
		var sy = h / (size.y + 1);
		for (var iw = 0; iw <= size.x; iw++) {
			for (var ih = 0; ih <= size.y; ih++) {
				var idx = (iw * w + ih) * 3;
				grids[idx] = sx * iw - iw -w / 2;
				grids[idx + 1] = sy * ih - h / 2;
				grids[idx + 2] = 0;
			}
		}

		var geometry = new THREE.BufferGeometry();
		geometry = new THREE.PlaneBufferGeometry(w, h, (size.x + 1, size.y + 1));
		// geometry.addAttribute( 'worldp', new THREE.BufferAttribute( grids, 3 ) );
		geometry.addAttribute( 'position', new THREE.BufferAttribute( grids, 3 ) );
		m = new THREE.Mesh(geometry, materail);
		 */

		// working case
		// works for w = h = 40, gl_Position = vec4(position)
		m = new THREE.Mesh( new THREE.PlaneBufferGeometry(
			w, h, (size.x + 1), (size.y + 1) ), materail );


		var positions = m.geometry.getAttribute('position');
		// var geometry = new THREE.BufferGeometry();
		var geometry = m.geometry;
		geometry.addAttribute( 'position', positions.clone() );
		geometry.addAttribute( 'worldp', positions.clone() );
		// geometry.attributes.position.setDynamic( true );

		// positions = geometry.attributes.worldp;
		// var worldp = geometry.attributes.worldp;
		// for (var iw = 0; iw <= size.x; iw++) {
		// 		var idx = (iw * w + ih) * 3;
		// 		positions[idx] = sx * iw - iw -w / 2;
		// 		positions[idx + 1] = sy * ih - h / 2;
		// 		positions[idx + 2] = 0;
		// 	}
		// }
