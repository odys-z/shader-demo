...
function OsmUtils () {
	 ...
	 */
	this.rad2cart = function (long, lat, a) {
	...
	this.long2tile = function (lon, zoom1) {
	    tt = Number(lon);
	    return (Math.floor((tt + 180) / 360 * Math.pow(2, zoom1)));
	}

	this.lat2tile = function (lat, zoom2)  {
	    return (Math.floor((1 - Math.log(Math.tan(lat * Math.PI / 180)
	        + 1 / Math.cos(lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, zoom2)));
	}

	this.tile2long = function (x, z) {
	    return (x / Math.pow(2, z) * 360 - 180);
	 }

	this.tile2lat = function (y, z) {
	 ...
	 * @return {object} tiles
	 */
	this.collectOsmTiles = function (tiles, dir, c0, a) {
	 ...
	 */
	this.loadImgTile = function (id, xyz) {
	 ...
	 */
	this.castPosition = function (eye, dir, r, c0) {
		if (c0 === undefined) {
			c0 = [0, 0, 0];
		}
	  ...
	  * @return distance
	  */
	this.distSphere = function ( eye, l, r, cent ) {
	 ...
	 */
	this.osmGlobeTiles = function (zoom, a) {
